...
| `tn_consensus` | DAG, Bullshark commit rule, proposer/voter/node machines | 🚧 part 1 done (aggregators, DAG) |
| `tn_execution` | execution seam (`Noop`, later an OCaml EVM) | ⏳ planned |
| `tn_sim` + `bin/tn_sim` | discrete-event simulator + CLI (the runnable slice) | ⏳ planned |
...
Current suite: **61 checks green** — 24 BCS golden-vector conformance checks,
12 foundation cases (crypto, scalars, committee threshold table), 9
vertex/certificate cases (the full assembly rejection matrix), and 16 consensus
cases (vote and parent aggregators, and the DAG equivocation / parent /
garbage-collection invariants mirrored from the Rust `dag_state_tests`).
...
as `dune exec tn_sim -- --validators 4 --seed 7 --until-s 60`. Steps 1–6 below
are done; 7–14 remain. This plan was produced and adversarially reviewed by a
multi-agent architecture pass; the HIGH-severity traps it surfaced are noted.
...
8. ⏳ `tn_consensus` part 2 — `Leader_schedule`, `Sub_dag` (reserve the
   reputation-scores digest field), `Bullshark`, output chain; replay Rust
   `bullshark_tests`
9. ⏳ `tn_consensus` part 3 — the machines: `Proposer` (**timer generation
   counters** to discard stale re-arms), `Voter` (vote-once, parent checks)
...
11. ⏳ `tn_sim` — event queue, seeded latency, batch injection; integration test
12. ⏳ `bin/tn_sim` — the runnable CLI vertical slice
