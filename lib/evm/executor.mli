(** The single-transaction state-transition executor: the Prague mainnet transaction
    STF.
    ...
    order, reimburses the caller and rewards the beneficiary, and returns a
    {!Receipt.t} beside the new world.
    ...
    prunes an {!Tn_state.Account.is_absent} account, so a zero-value call to a fresh
    address and a zero-reward beneficiary leave no empty account behind. The state
    root that would commit the pruned map is still downstream (reth), as everywhere
    in this port. *)
